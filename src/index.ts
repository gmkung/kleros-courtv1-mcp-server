...
async function fetchMetaEvidence(chainId: number, disputeId: string): Promise<{ metaEvidence: MetaEvidence; fileUrl: string }> {
...
async function fetchEvidences(chainId: number, disputeId: string): Promise<Evidence[]> {
...
async function fetchEvidenceContent(evidenceUri: string): Promise<EvidenceContent> {
  const url = `${KLEROS_CDN_BASE}${evidenceUri}`;
  const response = await axios.get(url);
  return response.data;
}

async function getDisputeData(disputeId: string, chainId: number): Promise<DisputeData> {
  ...
  const evidenceContents = await Promise.all(
    evidences.map(evidence => fetchEvidenceContent(evidence.URI))
  );
  
  // Generate evidence file URLs
  const evidenceFileUrls = evidenceContents.map(content => 
    content.fileURI ? `${KLEROS_CDN_BASE}${content.fileURI}` : ''
  ).filter(Boolean);
