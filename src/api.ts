...
export async function fetchMetaEvidence(
  chainId: number,
  disputeId: string
): Promise<{ metaEvidence: RawMetaEvidence; fileUrl: string }> {
  const response = await axios.get(
    `${KLEROS_API_BASE}/get-dispute-metaevidence`,
    {
  ...
  const metaEvidenceResponse = await axios.get<RawMetaEvidence>(metaEvidenceUrl, {
...
export async function fetchEvidenceContent(
  ...
  const response = await axios.get<EvidenceContent>(url, {
    headers: { 'Accept': 'application/json' }
  });
...
export async function getDisputeData(
  ...
  const processedMetaEvidence: MetaEvidence = {
  ...
  // Process results
  const successfulEvidenceContents: EvidenceContent[] = [];
  const evidenceErrors: Array<{ evidenceUri: string; error: string }> = [];
